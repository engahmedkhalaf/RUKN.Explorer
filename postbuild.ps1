...
Copy-Item -Path (Join-Path $SolutionDir "RUKN.Search.Common\Images\*") -Destination $imagesDir -Force
Copy-Item -Path (Join-Path $SolutionDir "RUKN.Search.Common\en-US\PluginRibbon.xaml") -Destination $enUsDir -Force
Copy-Item -Path (Join-Path $SolutionDir "RUKN.Search.Common\PackageContents.xml") -Destination $bundleDir -Force
